...
int main(int argc, char* argv[]){
  ...
  float minimum_ratio = stof(argv[4]);
  int M = stoi(argv[3]);
  ...
  while(execute_num-- != 0){
    if (pid==0) {
      ...
      if(rw_time_flag) {
        ...
        cout << "RW: " << (double)(tmp_time - rw_start)/CLOCKS_PER_SEC << endl;
        vector<int> rw_node(rw.begin(), rw.end());
        Graph time_test;
        g.SubGraphTest2(time_test,rw_node);
        rw_end = clock();
      ...
      clock_t check_tmp = clock();
      cout << "!!!" << (double)(check_tmp - start)/CLOCKS_PER_SEC << endl;
      // RW finish
      ...
      if (execute_num == 0) {
        ...
        int out_range = stoi(argv[5]) * 0.1;
        for (size_t i = out_range; i < stoi(argv[5])-out_range; i++) {
        ...
        if (pid < M/2) {
          ...
          p = pid * (2.0 * (minimum_ratio - 1) / M) + (2.0 - minimum_ratio);
        }
